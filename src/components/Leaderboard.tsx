...
export function Leaderboard({ currentUserId, activeSport: initialSport }: LeaderboardProps) {
  ...
  const [isCollapsed, setIsCollapsed] = useState(() => {
    try {
      return localStorage.getItem('salami_hide_leaderboard') === 'true';
    } catch {
      return false;
    }
  });
  ...
  const toggleCollapse = () => {
    ...
    try {
      localStorage.setItem('salami_hide_leaderboard', String(newVal));
    } catch {}
  };
  ...
  return (
    <div className={cn(
      "dashboard-card border-slate-800 bg-slate-900/80 backdrop-blur-md relative overflow-hidden animate-in fade-in duration-300 transition-all duration-300",
      isCollapsed ? "p-4" : "p-6"
    )}>
      ...
      <div className={cn("flex items-center justify-between", !isCollapsed && "mb-5")}>
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-2.5 py-1 text-[9px] font-mono font-black tracking-wider uppercase rounded border border-slate-800 bg-slate-950/60 text-slate-400 hover:text-white hover:bg-slate-900 transition-all hover:border-slate-700 cursor-pointer shadow-sm select-none"
          title={isCollapsed ? "Show Leaderboard" : "Hide Leaderboard"}
        >
          {isCollapsed ? (
            <>
            ...
            <>
              <EyeOff className="w-3 h-3 text-slate-550" />
              <span>Hide</span>
            </>
          )}
        </button>
      ...
      <AnimatePresence initial={false}>
        {!isCollapsed && (
          ...
          >
            {/* Toggles & Filter Grid */}
            <div className="space-y-3 mb-4 pt-3 border-t border-slate-800/20">
            ...
            {/* Banner info */}
            <div className="mt-4 pt-3 border-t border-slate-900/60 flex items-center justify-between text-[9px] font-mono text-slate-500 uppercase tracking-widest">
